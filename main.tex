...
using a Markov Decision Process (MDP) formulation. I build a trace-driven workload from the
Google Cluster Trace (2011) by extracting task arrivals, CPU/memory requests, and runtimes from
raw event logs. I implement an event-driven simulator in Python to evaluate (i) static provisioning,
...
VM-hour cost to heuristic autoscaling. I additionally apply the same trace-driven simulation pipeline to an Alibaba OpenB pod trace (Kaggle),
...
I apply the following simplifications while preserving the core trade-off:
\begin{enumerate}[leftmargin=1.2cm]
  ...
  \item \textbf{Clean lifecycle filtering:} I keep tasks with a single unambiguous
...
\section{Source}
I use the Google Cluster Trace (2011, version 2), which records task lifecycle events and
...
I convert raw event logs into a compact task table:
\[
...
I filter tasks to those with \(d>0\), \(d \le 86400\) seconds, and \(0 < cpu\_req, mem\_req \le 1\).

\subsection{Trace subset and window selection}
To keep processing manageable, I download a subset of consecutive \texttt{task\_events} shards,
...
I simulate a pool of identical VMs, each with normalized capacity:
\[
...
I implement a discrete-event simulation:
\begin{itemize}[leftmargin=1.2cm]
...
\subsection{Cost}
I measure cost as the time integral of active VM count:
\[
...
Every \(\Delta=60\) seconds I compute an aggregated queue backlog signal:
\[
Q(t) = \sum_{i \in \text{queue}} \max(c_i, m_i)\cdot d_i,
\]
interpretable as queued \emph{resource-seconds} of dominant demand.

If \(Q(t)\) exceeds an upper threshold, I scale up by a step size; if it falls below a lower
threshold, I scale down by a step size. Step sizes are used to ensure responsiveness under
bursty demand.
...
project. Instead, I use an aggregated MDP approximation:
\begin{itemize}[leftmargin=1.2cm]
...
\end{itemize}
I learn the policy using tabular Q-learning on the aggregated MDP, then evaluate it in the
detailed event-driven simulator.

% ============================================================
\chapter{Experiments}
\section{Experimental setup}
I evaluate on the busiest 2-hour window extracted from the Google trace. Control interval is
...
I evaluate three allocation strategies on the busiest 2-hour window extracted from the Google Cluster Trace
...
To ensure fair comparisons, I perform matched-SLA baselining: for each autoscaling policy I select a static capacity that
...
\begin{itemize}[leftmargin=1.2cm]
  ...
  \item \textbf{Requests vs usage:} I use requested CPU/memory, not actual utilization.
...
\section{Source}
To satisfy the assignment requirement of using Alibaba traces, I use the \emph{Alibaba OpenB pod trace}
available via Kaggle \citep{alibabaopenbkaggle}. The dataset contains pod-level resource requests and lifecycle
timestamps, enabling conversion to the same task format used in my simulator.

\section{Raw schema used}
I use the following fields from the pod list tables (e.g., \texttt{openb\_pod\_list\_default.csv}):
\begin{itemize}[leftmargin=1.2cm]
...
\end{itemize}
and I use node capacity fields from \texttt{openb\_node\_list\_all\_node.csv}:
\texttt{cpu\_milli} and \texttt{memory\_mib}.

\section{Preprocessing and normalization}
I convert each pod record into:
\[
(\text{arrival\_time},\ \text{runtime},\ cpu\_req,\ mem\_req).
\]
I set:
...
To match my homogeneous VM simulator, I normalize CPU and memory requests by node capacities observed in the trace:
\(C^{cpu} = 32000\) millicores and \(C^{mem} = 262144\) MiB (from \texttt{openb\_node\_list\_all\_node.csv}),
so that \(cpu\_req, mem\_req \in (0,1]\). I filter to pods with positive runtime and requests that fit within
...
too few events for stable autoscaling evaluation, so I select the busiest 24-hour window using a sliding-window
...
Because many pods start immediately when sufficient capacity is provisioned, p95 can become 0; therefore I also report
...
simulator, I quantify the cost--performance trade-offs between static provisioning, heuristic threshold scaling,
